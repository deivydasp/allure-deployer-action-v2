...
- **Environment widget workaround**: Always writes an empty `widgets/allure_environment.json` (in both report root and `awesome/` subdirectory) if missing. Allure 3.4+ stopped generating this file when no `environment.properties` is provided, but the awesome SPA still fetches it — a 404 crashes individual test result views with `Cannot read properties of undefined (reading 'message')`. See `ensureEnvironmentWidget` in `allure-report.service.ts`.
