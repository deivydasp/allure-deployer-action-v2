...
- **Allure version pinned to `~3.3.1`**: Versions 3.4+ introduced a breaking change to the awesome SPA — opening individual test results crashes with `Cannot read properties of undefined (reading 'message'/'payload')` due to changed `widgets/environments.json` shape. Upstream issue: [allure-framework/allure3#571](https://github.com/allure-framework/allure3/issues/571). Do not bump allure minor/major until that issue is fixed upstream.
