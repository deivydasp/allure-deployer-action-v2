...
async function runSummaryMode() {
    try {
        ...
        const summaryPageLink = `<table><tr><td><img src="https://raw.githubusercontent.com/deivydasp/allure-deployer-action-v2/master/assets/allure-logo.svg" width="20" height="20" /></td><td><a href="${normalizeUrl(pagesUrl)}">Summary Page</a></td></tr></table>`;
...
async function detectReruns(reportDir, pagesUrl, pagesSourcePath) {
    ...
    if (!runAttempt || runAttempt <= 1 || !inputs.prefix)
        return undefined;
    try {
...
async function scanSinglePrefix(prefixDir, dirName, pagesUrl, pagesSourcePath) {
    ...
    if (!entryStat?.isDirectory())
        return undefined;
    const runs = await readdir(prefixDir).catch((e) => {
